...
int ntrip_login(int fd)
{

	int i = 0;
	char buf[512] = {0};
	i = sprintf(buf, "%s%s%s%s%s", "GET /CUT00 HTTP/1.0\r\n",
									"User-Agent: NTRIP GNSSInternetRadio/1.4.10\r\n",
									"Accept: */*\r\n",
									"Connection: close\r\n",
									"Authorization: Basic Q1VNVEJaSjpaSjE5OTUxMTEw\r\n");
	if (send(fd, buf, strlen(buf), 0);
	printf("%s", buf);
	return 0;
}
...
void* thread_handler(void* args)
{
	...
	{
		printf("connect server success! fd:%d\n", sockfd);
	}
